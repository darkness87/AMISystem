...
public abstract class DashBoardDAOImpl implements DashBoardDAO {
	...
	public List<DashBoardVO> findById(long id) {

		log.info("testDAOImple TestStart : {}", id);

		CriteriaBuilder criteriaBuilder = entityManager.getCriteriaBuilder();

		CriteriaQuery<DashBoardVO> criteriaQuery = criteriaBuilder.createQuery(DashBoardVO.class); // 값을 가져오기 원하는 클래스 추가

		Root<DashBoardVO> root = criteriaQuery.from(DashBoardVO.class); // Root => SQL FROM 구문과 유사
		...
		TypedQuery<DashBoardVO> boardListQuery = entityManager.createQuery(criteriaQuery); // 생성된 쿼리문으로 리턴

		List<DashBoardVO> testData = boardListQuery.getResultList(); // 결과값 객체 담기
