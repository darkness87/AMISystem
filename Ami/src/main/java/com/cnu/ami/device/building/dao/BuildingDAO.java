...
public interface BuildingDAO extends JpaRepository<BuildingEntity, Long> { // 키 값이 숫자 일경우 Long, 문자열 String
	...
	@Query(value = "SELECT AA.BSEQ AS BSEQ,AA.GSEQ AS GSEQ,AA.BNAME AS BNAME,AA.GID AS GID,AA.GNAME AS GNAME,BB.DID AS DID,BB.S_SYS_STATE AS S_SYS_STATE\r\n"
			+ "FROM (SELECT A.BSEQ,A.GSEQ,A.BNAME,B.GID,B.GNAME\r\n" + "FROM BUILDING AS A\r\n"
	...
	@Query(value = "SELECT AA.BSEQ,AA.GSEQ,AA.BNAME,AA.GID,AA.GNAME,BB.DID,BB.S_SYS_STATE\r\n"
			+ "FROM (SELECT A.BSEQ,A.GSEQ,A.BNAME,B.GID,B.GNAME\r\n" + "FROM BUILDING AS A\r\n"
