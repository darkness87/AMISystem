...
public interface DcuInfoDAO extends JpaRepository<DcuInfoEntity, String> { // 키 값이 숫자 일경우 Long, 문자열 String
	...
	@Query(value = "SELECT AA.BSEQ,AA.GSEQ,AA.BNAME,AA.GID,AA.GNAME,AA.CNT_METER,AA.CNT_MODEM,AA.RNAME,BB.DID,BB.DCU_IP,BB.FEP_IP,BB.FWV,BB.S_SYS_STATE\r\n"
			+ "FROM (SELECT A.BSEQ,A.GSEQ,A.BNAME,B.GID,B.GNAME,B.CNT_METER,B.CNT_MODEM,B.RNAME\r\n"
			+ "FROM BUILDING AS A\r\n"
			+ "LEFT JOIN (SELECT T1.GSEQ,T1.RSEQ,T1.GID,T1.GNAME,T1.CNT_METER,T1.CNT_MODEM,T2.RNAME FROM GROUPSET AS T1\r\n"
