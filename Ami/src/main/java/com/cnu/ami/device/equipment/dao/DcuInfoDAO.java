...
public interface DcuInfoDAO extends JpaRepository<DcuInfoEntity, String> { // 키 값이 숫자 일경우 Long, 문자열 String
	...
	@Query(value = "SELECT AA.BNAME,AA.GID,AA.GNAME,IFNULL(AA.CNT_METER,0) AS CNT_METER,IFNULL(AA.CNT_MODEM,0) AS CNT_MODEM,AA.RNAME,BB.DID,BB.DCU_IP,BB.DCU_PORT,BB.FWV,BB.S_SYS_STATE,BB.DSTATUS\r\n"
			+ "FROM (SELECT A.BSEQ,A.GSEQ,A.BNAME,B.GID,B.GNAME,B.CNT_METER,B.CNT_MODEM,B.RNAME\r\n"
			+ "FROM BUILDING AS A\r\n"
			+ "JOIN (SELECT T1.GSEQ,T1.RSEQ,T1.GID,T1.GNAME,T1.CNT_METER,T1.CNT_MODEM,T2.RNAME FROM GROUPSET AS T1\r\n"
			+ "JOIN REGION AS T2 ON T1.RSEQ=T2.RSEQ WHERE T1.GSEQ=:gseq) AS B\r\n" + "ON A.GSEQ=B.GSEQ) AA JOIN \r\n"
			+ "(SELECT C.DID,C.DCU_IP,C.DCU_PORT,C.ROUTER_IP,C.FWV,C.S_SYS_STATE,C.DSTATUS,D.BSEQ FROM DCU_INFO AS C\r\n"
