...
public interface MappingDAO extends JpaRepository<HouseEntity, String> {
	...
	@Query(value = "SELECT T1.GSEQ,T1.BSEQ,T6.HSEQ,T1.BNAME,T6.HO,T4.METER_ID,T4.MRD,T4.MAC,T3.DID,T7.MTYPE,IFNULL(T7.MTIME,0) AS MTIME\r\n" + 
			...
			"ON T3.DID=T4.DID\r\n" + 
			"JOIN (SELECT HSEQ,METER_ID FROM HOUSE_METER_MAP) AS T5\r\n" + 
			"ON T4.METER_ID=T5.METER_ID\r\n" + 
			"JOIN (SELECT HSEQ,HO FROM HOUSEHOLD WHERE GSEQ = :gseq) AS T6\r\n" + 
			"ON T5.HSEQ=T6.HSEQ\r\n" + 
