...
@Repository
public interface FBoardDAO extends JpaRepository<RealTimeEntity, Long> { // 키 값이 숫자 일경우 Long, 문자열 String

	@Query(value = "SELECT T1.RSEQ,T1.RNAME,IFNULL(T2.HOUSECOUNT,0) AS COUNT\r\n" + 
			"FROM (SELECT RSEQ,RNAME FROM REGION) AS T1\r\n" + 
			"LEFT JOIN (SELECT RSEQ,SUM(CNT_HOUSE) AS HOUSECOUNT FROM GROUPSET GROUP BY RSEQ) AS T2\r\n" + 
			"ON T1.RSEQ=T2.RSEQ\r\n" + 
			"ORDER BY RSEQ", nativeQuery = true)
	List<FBoardCountInterfaceVO> getHouseCount();
	...
	@Query(value = "SELECT S1.RSEQ,S1.RNAME,IFNULL(SUM(S2.DCUCOUNT),0) AS COUNT\r\n" + 
			...
			"LEFT JOIN (SELECT RSEQ,GSEQ,GNAME FROM GROUPSET) AS G2\r\n" + 
			...
			"GROUP BY RSEQ\r\n" + 
			"ORDER BY RSEQ", nativeQuery = true)
	List<FBoardCountInterfaceVO> getDcuCount();
	...
	@Query(value = "SELECT M1.RSEQ,M1.RNAME,IFNULL(SUM(M2.METERCOUNT),0) AS COUNT\r\n" + 
			...
			"GROUP BY RSEQ\r\n" + 
			"ORDER BY RSEQ", nativeQuery = true)
	List<FBoardCountInterfaceVO> getMeterCount(@Param("mtime") long mtime);
