...
public interface RealTimeDAO extends JpaRepository<RealTimeEntity, Long> {
	...
	@Query(value = "SELECT T2.RSEQ,T1.GSEQ,T1.BSEQ,T2.RNAME,T2.GID,T2.GNAME,T1.BNAME,T4.HO,T1.DID,T3.METER_ID,T3.MAC,IFNULL(T5.MTIME,0) AS MTIME,T5.FAP,T5.RFAP,T5.MTYPE,T5.UDATE\r\n"
