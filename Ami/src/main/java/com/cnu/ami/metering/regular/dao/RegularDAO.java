...
public interface RegularDAO extends JpaRepository<RegularMonthEntity, Long> {
	...
	@Query(value = "SELECT T2.RSEQ,T1.GSEQ,T1.BSEQ,T2.RNAME,T2.GID,T2.GNAME,T1.BNAME,T4.HO,T1.DID,T3.METER_ID,T3.MAC,T5.ITIME,T5.MTIME,T5.APT1,T5.APT2,T5.RPT,T5.LPT,T5.PFT FROM(SELECT A.BSEQ,A.GSEQ,A.BNAME,B.DID FROM BUILDING AS A\r\n"
			...
			+ "ON T3.METER_ID=T4.METER_ID\r\n" + "LEFT JOIN GAUGE_PERIOD AS T5\r\n" + "ON T3.METER_ID=T5.METER_ID\r\n"
			+ "ORDER BY T1.BNAME ASC, T1.DID ASC, T4.HO ASC\r\n" + "", nativeQuery = true)
	List<RegularMonthInterfaceVO> getRegularData(@Param("gseq") int gseq);
