...
public interface RegularDAO extends JpaRepository<RegularMonthEntity, Long> {
	...
	@Query(value = "SELECT T2.RNAME,T2.GID,T2.GNAME,T1.BNAME,T4.HO,T1.DID,T3.METER_ID,IFNULL(T3.MRD,0) AS MRD,T3.MAC,\r\n" + 
			"IFNULL(T5.MTIME,0) AS FROM_MTIME, IFNULL(T5.APT1,0) AS FROM_APT1, IFNULL(T5.R_APT1,0) AS FROM_R_APT1, IFNULL(T6.MTIME,0) AS TO_MTIME, IFNULL(T6.APT1,0) AS TO_APT1, IFNULL(T6.R_APT1,0) AS TO_R_APT1, IFNULL(T6.APT1-T5.APT1,0) AS F_USE, IFNULL(T6.R_APT1-T5.R_APT1,0) AS R_USE\r\n" + 
			...
			"LEFT JOIN (SELECT METER_ID,MTIME,APT1,R_APT1 FROM GAUGE_PERIOD WHERE FROM_UNIXTIME(MTIME) = :fromDate) AS T5 ON T3.METER_ID=T5.METER_ID\r\n" + 
			"LEFT JOIN (SELECT METER_ID,MTIME,APT1,R_APT1 FROM GAUGE_PERIOD WHERE FROM_UNIXTIME(MTIME) = :toDate) AS T6 ON T3.METER_ID=T6.METER_ID\r\n" + 
			"ORDER BY T1.BNAME ASC, T1.DID ASC, T4.HO ASC" + "", nativeQuery = true)
	List<RegularMonthInterfaceVO> getRegularData(@Param("gseq") int gseq,@Param("fromDate") String fromDate,@Param("toDate") String toDate);
