...
public class SecurityConfig extends WebSecurityConfigurerAdapter {
	...
	// TODO 참조 https://bamdule.tistory.com/53
	...
	protected void configure(HttpSecurity http) throws Exception {
	  ...
	  .and()
	  	.authorizeRequests() // 다음 Request에 대한 사용권한 체크
	  		.antMatchers("/api/login", "/api/registration", "/api/getApiList").permitAll() // 가입 및 인증 주소는 누구나 접근가능 // 추후 누구나 접근가능한 컨트롤러 설정 후 /* 표시로 한번에 설정 고려
//	  		.anyRequest().hasRole("ADMIN") // 그외 나머지 요청은 모두 인증된 회원만 접근 가능
	  		.anyRequest().hasAnyRole("ADMIN","USER") // 추후 hasRole , hasAnyRole 에 따라 API 접근 권한 변경
	  .and()
